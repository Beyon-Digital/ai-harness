//! Catalogued event staging for timer streams.
//!
//! Mirrors the runtime/effects staging helpers: sensitivity and retention
...
/// Stages a catalogued timer event through the transactional outbox.
pub(crate) async fn stage_timer_event(
    ...
    let stream_key = EventStreamKey::new(stream.as_str().to_owned()).map_err(|_| {
        KernelError::new(
            ErrorCode::Internal,
            RetryClass::Never,
            "catalogued timer stream key is not canonical",
        )
    })?;
    stage(
        txn,
