...
- macOS: `APPLE_CERTIFICATE` (base64 `.p12`),
  ...
  `APPLE_API_KEY` (App Store Connect key ID), `APPLE_API_ISSUER`,
  `APPLE_API_PRIVATE_KEY` (base64 `AuthKey_<id>.p8`), and
  `KEYCHAIN_PASSWORD`.
...
Connect API key, Authenticode-signs Windows installers, and fails the
release if required credentials are absent.
